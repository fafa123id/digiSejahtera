...
class DemoKoperasiSeeder extends Seeder
{
    public function run(): void
    {
        DB::transaction(function (): void {
            ...
            $bulanSekarang = CarbonImmutable::now()->startOfMonth();

            $sparxie = $this->buatAnggota(
                nomorAnggota: '001',
                nama: 'sparxie',
                agama: 'islam',
                tanggalMasuk: $bulanSekarang->subYears(2)->startOfYear()
            );
            ...
            $yaoguang = $this->buatAnggota(
                ...
                nama: 'yaoguang',
                agama: 'islam',
                tanggalMasuk: $bulanSekarang->subYears(1)->startOfYear()
            );
            ...
            $this->perbaruiRekapSimpanan($sparxie);
            $this->perbaruiRekapSimpanan($yaoguang);
            ...
            $this->perbaruiShu($sparxie);
            $this->perbaruiShu($yaoguang);
        });
    }
    ...
    private function buatPengurus(): User
    {
        return User::updateOrCreate(
            ...
            [
                ...
                'username' => 'pengurus',
                'password' => Hash::make('pengurus12345'),
    ...
    private function buatSimpanansparxie(Anggota $anggota, CarbonImmutable $bulanSekarang): void
    {
        $this->simpanSimpanan(
            anggota: $anggota,
            periode: $bulanSekarang->subMonths(3),
        ...
        $this->simpanSimpanan(
            anggota: $anggota,
            periode: $bulanSekarang->subMonths(2),
            simpananPokok: 0,
            simpananWajib: 50_000,
            simpananSukarela: 25_000,
            simpananHariRaya: 100_000,
        ...
        $this->simpanSimpanan(
            anggota: $anggota,
            periode: $bulanSekarang->subMonth(),
            simpananPokok: 0,
            simpananWajib: 50_000,
            simpananSukarela: 25_000,
            simpananHariRaya: 100_000,
    ...
    private function buatSimpananyaoguang(Anggota $anggota, CarbonImmutable $bulanSekarang): void
    {
        $this->simpanSimpanan(
            anggota: $anggota,
            periode: $bulanSekarang->subMonths(3),
    ...
    private function perbaruiRekapSimpanan(Anggota $anggota): RekapSimpanan
    {
        $query = Simpanan::query()->where('anggota_id', $anggota->id);

        $totalPokok = (float) (clone $query)->sum('simpanan_pokok');
        $totalWajib = (float) (clone $query)->sum('simpanan_wajib');
        $totalSukarela = (float) (clone $query)->sum('simpanan_sukarela');
        $totalHariRaya = (float) (clone $query)->sum('simpanan_hari_raya');
        $totalRekreasi = (float) (clone $query)->sum('simpanan_rekreasi');
        ...
        return RekapSimpanan::updateOrCreate(
            ...
            [
                ...
                'total_simpanan_hari_raya' => $totalHariRaya,
                'total_simpanan_rekreasi' => $totalRekreasi,
                'total_simpanan' => $totalPokok + $totalWajib + $totalSukarela + $totalHariRaya + $totalRekreasi,
            ]
        );
    }

    private function buatPinjamanRegulersparxie(Anggota $anggota, CarbonImmutable $bulanSekarang): void
    {
        $tanggalPinjaman = $bulanSekarang->subMonths(3);
        ...
        $pinjaman = Pinjaman::updateOrCreate(
            ...
            [
                'nominal_pinjaman' => 20_000_000,
                'persentase_jasa' => 1.50,
                'sisa_pinjaman' => 15_500_000,
                'status' => Pinjaman::STATUS_AKTIF,
                'keterangan' => 'Pinjaman reguler demo',
            ]
        );
    ...
    private function buatPinjamanSebrakyaoguang(Anggota $anggota, CarbonImmutable $bulanSekarang): void
    {
        $tanggalPinjaman = $bulanSekarang->subMonths(2);
        ...
        $pinjaman = Pinjaman::updateOrCreate(
            ...
            [
                'nominal_pinjaman' => 5_000_000,
                'persentase_jasa' => 2.00,
                'sisa_pinjaman' => 5_000_000,
                'status' => Pinjaman::STATUS_AKTIF,
                'keterangan' => 'Pinjaman sebrak demo',
            ]
        );
    ...
    private function simpanAngsuran(
        ...
        $persentaseJasa = (float) $pinjaman->persentase_jasa;
        $jasaPinjaman = $saldoAwal * ($persentaseJasa / 100);
        $sisaPinjaman = $saldoAwal - $nominalAngsuran;
        ...
        return Angsuran::updateOrCreate(
            ...
            [
                'tanggal_pembayaran' => $periode->addDays(5)->toDateString(),
    ...
    private function perbaruiShu(Anggota $anggota): ShuAnggota
    {
        $totalSimpanan = (float) RekapSimpanan::query()
            ->where('anggota_id', $anggota->id)
            ->value('total_simpanan');

        $totalJasaPinjaman = (float) Angsuran::query()
            ->whereHas('pinjaman', function ($query) use ($anggota): void {
                $query->where('anggota_id', $anggota->id);
            })
            ->sum('jasa_pinjaman');

        $shuSimpanan = $totalSimpanan * 0.50;
        ...
        return ShuAnggota::updateOrCreate(
            ...
            [
                ...
                'total_shu' => $shuSimpanan + $shuPinjaman,
                'calculated_at' => now(),
            ]
        );
    }
